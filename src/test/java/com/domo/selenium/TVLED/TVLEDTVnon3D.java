...
public class TVLEDTVnon3D {
	...
	public void TestTVnon3D() throws InterruptedException, IOException
	{
		...
		if (h.isElementPresent(By.id("CB_3_Nu"))==false)
		{
			logger.Log(LOG_FILE, "TestTVnon3D(): Option is not available TEST FAILED !!!");
			h.screenShooter("TestTVnon3D", d);
		}
		assertTrue(h.isElementPresent(By.id("CB_3_Nu"))==true);
		...
		while (b!=true)
		{
			...
			{	
				//procesez
				if (d.findElement(By.xpath(String.format(Constants.domo_product_details_2_xpath, i,j))).getText().contains(Constants.TVNON3D)==false)
				{
					logger.Log(LOG_FILE, "TestTVNON3D(): TEST FAILED !!!");
					h.screenShooter("TestTVNON3D", d);
				}
				assertTrue(d.findElement(By.xpath(String.format(Constants.domo_product_details_2_xpath, i,j))).getText().contains(Constants.TVNON3D));
				//procesez
