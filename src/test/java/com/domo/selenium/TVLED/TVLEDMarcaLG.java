...
public class TVLEDMarcaLG {
	...
	public void TestTVLEDMarcaLG() throws InterruptedException, IOException
	{
		...
		{
			logger.Log(LOG_FILE, "TestTVLEDMarcaLG(): Wrong page TEST FAILED !!!");
		...
		if (h.isElementPresent(By.id("CB_0_LG"))==false)
		{
			logger.Log(LOG_FILE, "TestTVLEDMarcaLG(): Option is not available TEST FAILED !!!");
			h.screenShooter("TestTVLEDMarcaLG", d);
		}
		assertTrue(h.isElementPresent(By.id("CB_0_LG"))==true);
		...
		while (b!=true)
		{
			...
			{	
				...
				{
					logger.Log(LOG_FILE, "TestTVLEDMarcaLG(): TEST FAILED !!!");
					h.screenShooter("TestTVLEDMarcaLG", d);
				}
