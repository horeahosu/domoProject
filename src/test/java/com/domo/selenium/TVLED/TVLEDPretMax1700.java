...
public class TVLEDPretMax1700 {
	...
	/**
	 * TestTVLEDPretMax1700(): Verifica optiunea de a afisa pe pagina doar produsele avand pretul mai mic de 1700
	 * 
	 */
	...
	public void TestNotebookPretMaxim2100() throws InterruptedException, IOException
	{
		...
		logger.Log(LOG_FILE, "TestTVLEDPretMax1700(): Verifica optiunea de a afisa pe pagina doar produsele avand pretul mai mic de 1700");
		h.waitForElementPresent(By.xpath(Constants.domo_product_first_title_xpath), 5);
		if (d.findElement(By.xpath(Constants.domo_product_first_title_xpath)).getText().contains("LED")==false)
		{
			logger.Log(LOG_FILE, "TestTVLEDPretMax1700(): Wrong page TEST FAILED !!!");
			h.screenShooter("TestTVLEDPretMax1700", d);
		}
		assertTrue(d.findElement(By.xpath(Constants.domo_product_first_title_xpath)).getText().contains("LED"));
		
		if (h.isElementPresent(By.id("CB_1_Sub_1.700_lei"))==false)
		{
			logger.Log(LOG_FILE, "TestTVLEDPretMax1700(): Option is not available TEST FAILED !!!");
			h.screenShooter("TestTVLEDPretMax1700", d);
		}
		assertTrue(h.isElementPresent(By.id("CB_1_Sub_1.700_lei"))==true);			
				
		d.findElement(By.id("CB_1_Sub_1.700_lei")).click();
		Thread.sleep(2000);
		...
		while (b!=true)
		{
			...
			{	
				...
				if ((amount<Constants.TVLED_PRET_1700)==false)
				{
					logger.Log(LOG_FILE, "TestTVLEDPretMax1700(): A product with non coresponding price was found TEST FAILED !!!");
					h.screenShooter("TestTVLEDPretMax1700", d);
				}
				...
				{ if (h.isElementPresent(By.id("NextPage2"))==true && (d.findElement(By.id("NextPage")).isDisplayed()==true))
					...
					{
						b=true;
						logger.Log(LOG_FILE, "Numarul de produse avand pretul mai mic de 1700 sunt: "+k);
						logger.Log(LOG_FILE, "TestTVLEDPretMax1700(): TEST PASSED");
						i=3;
						j=1;
					}
				}
