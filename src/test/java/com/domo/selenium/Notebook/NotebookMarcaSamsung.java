...
public class NotebookMarcaSamsung {
	...
	/**
	 * TestNotebookMarcaSamsung(): Verifica optiunea de a afisa pe pagina doar produsele avand marca Samsung
	 * 
	 */
	
	@Test
	public void TestNotebookMarcaSamsung() throws InterruptedException, IOException
	{
		logger.Log(LOG_FILE, "TestNotebookMarcaSamsung(): Verifica optiunea de a afisa pe pagina doar produsele avand marca Samsung");
		...
		{
			logger.Log(LOG_FILE, "TestNotebookMarcaSamsung(): Option is not available TEST FAILED !!!");
			h.screenShooter("TestNotebookMarcaSamsung", d);
		}
			...
			{	
				...
				{ if (h.isElementPresent(By.id("NextPage2"))==true && (d.findElement(By.id("NextPage")).isDisplayed()==true))
					...
					else
					{
						...
						logger.Log(LOG_FILE, "TestNotebookMarcaSamsung(): TEST PASSED");
						i=1;
						j=1;
					}
				}
