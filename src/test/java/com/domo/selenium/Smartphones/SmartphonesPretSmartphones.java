...
public class SmartphonesPretSmartphones {
	...
	/**
	 * TestPretSmartphones(): Verifica pretul produselor HDD pe baza fisierului: PreturiProduse.xls
	 * 
	 */
	
	@Test
	public void TestPretSmartphones() throws InterruptedException, IOException
	{
		
		logger.Log(LOG_FILE, "TestPretSmartphones(): Verifica pretul produselor Smartphones pe baza fisierului: PreturiProduse.xls");
		...
		f = new FileUtil();
		c = f.getExcelRowsNumber("src/test/resources/PreturiProduse.xls", "SMARTPHONES");
		System.out.println("Numarul de produe Smartphones: "+c);
		cod = new String [c]; 
		pret = new int [c];
		...
		for(int i=0; i<c; i++)
		{
		cod [i]=f.readFromExcel("src/test/resources/PreturiProduse.xls", "SMARTPHONES", i, 0);
		//System.out.println(cod[i]);
		pret[i]= Integer.parseInt(h.SplitString3(f.readFromExcel("src/test/resources/PreturiProduse.xls", "SMARTPHONES", i, 1)));
		...
		while (b!=true)
		{
			...
			{	
				...
				logger.Log(LOG_FILE, "TestPretSmartphones(): Verify product:"+cod_site);
				for (int ii=0; ii<c; ii++)
				{
					...
					if ((pret[ii]==amount)==false)
					{
						...
						logger.Log(LOG_FILE, "TestPretSmartphones(): "+cod_site+" "+amount+" "+"MATCHED DATABASE");
					assertTrue(pret[ii]==amount);
					break;
					}
					if (ii+1==c)
						logger.Log(LOG_FILE, "TestPretSmartphones(): Product "+cod_site+" does not appear in DATABASE !!");	
				}
				//procesez
